...
(function () {
  ...
  function extractMovieData() {
    ...
    if (url.match(LETTERBOXD_MATCHER)) {
      ...
      // Find TMDb ID
      const tmdbLink = elementsArray.find((link) =>
        link?.href?.match(TMDB_MATCHER),
      );
      if (tmdbLink) {
  ...
  function attachBanner() {
    ...
    banner.addEventListener(
      "mousedown",
      (event) => event.button === 1 && openPlayer(true),
    );
